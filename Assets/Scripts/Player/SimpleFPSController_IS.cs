...
public class SimpleFPSController_IS : MonoBehaviour
{
    ...
    [Header("Animation")]
    [Tooltip("Animator on Character Child")]
    ...
    void Awake()
    {
        Debug.Log($"[LOCAL PLAYER SPAWN] {name} in scene {gameObject.scene.name}",
                  gameObject);
    }
    ...
    void Look()
    {
        ...
        if (cameraPivot) cameraPivot.localEulerAngles = new Vector3(_pitch, 0f, 0f);
        else playerCamera.transform.localEulerAngles = new Vector3(_pitch, 0f, 0f);
    }
    ...
    void Move()
    {
        ...
        var move = moveHoriz; move.y = _yVel;
